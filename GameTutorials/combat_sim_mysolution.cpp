...
//	int total; 			// total humans or monsters left after the battle
	int Dead_Humans = Humans;			// dead humans from the battle
	int Dead_Monsters = Monsters;	// dead monster from the battle
...
//	counter = Humans + Monsters; 
	while ((Dead_Humans > 0)  && (Dead_Monsters > 0)) {
		Human_attack = Attackpoints(RandomGenerator);
        	Monster_attack = Attackpoints(RandomGenerator); 
	...
	Remaining_Monsters = Monsters - Dead_Monsters;
	// Remaining humans after battle
	Remaining_Humans = Humans - Dead_Humans;
